...
LINDEX *combinar(LINDEX l1, LINDEX l2){
	LINDEX *merge = (LINDEX*) malloc(sizeof(LINDEX)); // Creamos una lista vacía
	merge->head = NULL;
	merge->tail = NULL;
	
	INDEX actual = l1->head; // Primero recorremos la primera lista
	while(actual->next != NULL){ // Mientras no lleguemos a tail
		append_index(merge, actual); // Añadimos el nodo a la lista merge
		actual = actual->next; // Avanzamos al siguiente nodo
	}
	append_index(merge, actual); // Añadimos el último nodo de la lista l1
	
	INDEX actual = l2->head; // Ahora recorremos la segunda lista
	while(actual->next != NULL){ // Mientras no lleguemos a tail
		append_index(merge, actual); // Añadimos el nodo a la lista merge
		actual = actual->next; // Avanzamos al siguiente nodo
	}
	append_index(merge, actual); // Añadimos el último nodo de la lista l2
...
void backpatch(CODE *cod, LINDEX l, char *label){
	...
	INDEX *index_actual = init_index(); // Creamos un índice vacío
